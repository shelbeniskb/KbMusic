...
var host = {
    ...
    doPlayAudio: function(songInfo) {
        var audio = this.defaultPlayer,
            songsList = this.songsList,
            len = songsList && songsList.length;
        ...
        if (len > 0) {
            ...
            this.doShowArtistImgStatic(songInfo);
        }
...
function validateAndSubmit(type) {
    var LOGIN = 0,
        REGISTER = 1,
        userName, userPwd, userPwdAgain, userEmail;
    ...
    if (type === REGISTER) {
        ...
        $.ajax({
            ...
            data: {userName: userName, userPwd: userPwd, userEmail: userEmail},
            success: function(data) {
                $('#overlay').css('display', 'none');
                $('#user-login-signup').css('display', 'none');
                $(document).off('click');
